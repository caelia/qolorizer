  ...
  (let-values (((r g b a) (parse-color color-spec alpha)))
    (let ((mode-spec (get-cairo-blend-mode blend-mode)))
      (lambda (src-file dest-file)
        (let* ((surf (cairo-image-surface-create-from-png src-file))
               (width (cairo-image-surface-get-width surf)) 
               (height (cairo-image-surface-get-height surf)) 
          ...
          (cairo-fill ctx)
          (cairo-surface-write-to-png surf dest-file)
